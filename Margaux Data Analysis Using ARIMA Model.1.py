...
data = pd.read_csv('Margaux.csv')
(printdata.head(7))

dateparse = lambda dates:pd.datetime.strptime(dates, '%d/%m/%Y')
data = pd.read_csv('Margaux.csv',parse_dates=['Date'],index_col='Date',date_parser=dateparse)
print('\n Parsed Data:')
print(data.head(7))
...
data=pd.read_csv('Margaux.csv', index_col=0, parse_dates=True)
...
# The Margaux vintage 2009 plot shows an .... pattern of rises and falls but which takes nearly 2 years. 
...
plt.legend(loc='best')
plt.title('Rolling Mean & Standard Deviation - Margaux 2007')
...
# Test Statistic > Critical Values. Therefore the null hypothesis is not rejected and the Margaux 2007 TS is Stationary.
...
ts_log = np.log(ts)
plt.plot(ts_log)
plt.title('Plot Log TS - Margaux 2007', fontsize = 15)
plt.xlabel('year', fontsize = 15)
...
plt.title('Plot Log TS & Moving Average - Margaux 2007', Fontsize = 15)
plt.xlabel('year', fontsize = 15)
...
plt.title ('Plot Log Time Series & Exponential Weighted -Margaux 2007', fontsize = 15)
plt.xlabel('year', fontsize = 15)
plt.ylabel('price', fontsize = 15)	
...
plt.title ('Plot Log Diff TS -Margaux 2007', fontsize = 15)
...
plt.title ('Original Margaux 2007', fontsize = 15)
plt.legend(loc='best')
plt.subplot(412)
plt.plot(trend, label='Trend')
plt.legend(loc='best')
plt.title ('Trend Margaux 2007', fontsize = 15)
plt.subplot(413)
plt.plot(seasonal,label='Seasonality')
plt.legend(loc='best')
plt.title ('Seasonaity Margaux', fontsize = 15)
plt.subplot(414)
plt.plot(residual, label='Residuals')
plt.legend(loc='best')
plt.title ('Residual Margaux 2007', fontsize = 15)
plt.tight_layout()
...
plt.title('Margaux - RMSE: %.4f'% np.sqrt(sum((predictions_ARIMA-ts)**2)/len(ts)))