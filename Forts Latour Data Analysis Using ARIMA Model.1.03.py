...
data = pd.read_csv('Latour.csv',parse_dates=['Date'],index_col='Date',date_parser=dateparse)
print('\n Parsed Data:')
print(data.head(7))
...
data=pd.read_csv('Latour.csv', index_col=0, parse_dates=True)
...
data.loc['2010-03-31']
data.loc['2009-03-31': '2013-10-30']
...
# The Lafite Rothschild vintage 2009 plot shows an irregular pattern of rises and falls but which takes nearly 2 years. Therefore, it is a cyclic Time-Series, with a decending trend.
...
# rolmean = ts.rolling(window=12).mean -- This gave error and I had to be fixed
# rolstd = ts.rolling(window=12).std  --- This part of the code gave error and I had to be fixed
...
plt.legend(loc='best')
plt.title('Rolling Mean & Standard Deviation - Latour 2007')
...
# Test Statistic > Critical Values. Therefore the null hypothesis is not rejected and the Latour 2007 TS is Stationary.
...
ts_log = np.log(ts)
plt.plot(ts_log)
plt.title('Plot Log TS - Latour 2007', fontsize = 15)
plt.xlabel('year', fontsize = 15)
...
plt.title('Plot Log TS & Moving Average - Latour 2007', Fontsize = 15)
plt.xlabel('year', fontsize = 15)
...
plt.title ('Plot Log Time Series & Exponential Weighted -Latour 2007', fontsize = 15)
plt.xlabel('year', fontsize = 15)
plt.ylabel('price', fontsize = 15)	
...
plt.title ('Plot Log Diff TS -Latour 2007', fontsize = 15)
...
plt.title ('Original Latour 2007', fontsize = 15)
plt.legend(loc='best')
plt.subplot(412)
plt.plot(trend, label='Trend')
plt.legend(loc='best')
plt.title ('Trend Latour 2007', fontsize = 15)
plt.subplot(413)
plt.plot(seasonal,label='Seasonality')
plt.legend(loc='best')
plt.title ('Seasonaity Latour', fontsize = 15)
plt.subplot(414)
plt.plot(residual, label='Residuals')
plt.legend(loc='best')
plt.title ('Residual Latour 2007', fontsize = 15)
plt.tight_layout()
...
# We can determine p and q values from the ACF and the PACF: p = 2 and q = 2
...
plt.title('LATOUR - RMSE: %.4f'% np.sqrt(sum((predictions_ARIMA-ts)**2)/len(ts)))
